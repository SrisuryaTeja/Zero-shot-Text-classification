...
def multi_positive_contrastive_loss(similarity, positive_mask):
    ...
    loss = -(log_probs * positive_mask).sum(1) / positive_mask.sum(1)
    return loss.mean()

def build_sampled_label_space(batch_positive_mask, all_train_labels, num_negatives):
    """
    batch_positive_mask: [B, L]
    all_train_labels: list of prompted train labels
    num_negatives: int
    """

    B, L = batch_positive_mask.shape

    # Find which labels are positive in batch
    ...
    # Final label subset
    final_indices = positive_indices + sampled_negatives
...
# Keep raw for indexing
    ...
    # Model
    # ----------------------------------------------
    if config["model"]["variant"] == "polyencoder":
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        while step < total_steps:
            ...
            for texts, positive_mask in train_loader:
                ...
                # Remap positive mask
                sampled_positive_mask = positive_mask[:, final_indices]
                
                sampled_positive_mask = sampled_positive_mask.to(device)

                # Compute similarity only on sampled labels
                similarity = model(texts, sampled_labels, device)

                loss = multi_positive_contrastive_loss(
                    similarity,
                    sampled_positive_mask.to(device)
                )
                ...
                if step % val_interval == 0:
                    ...
                    if avg_val_loss < best_val_loss:
                        ...
                        os.makedirs("poly-encoder-neg-60", exist_ok=True)
                        model.encoder.save_pretrained("poly-encoder-neg-60")
                        model.tokenizer.save_pretrained("poly-encoder-neg-60")
        ...
        # Save model
        # ----------------------------------------------
        mlflow.log_artifacts("poly-encoder")
        
        print("Training complete. Running zero-shot evaluation...")
        
        best_model = PolyEncoderModel(config["model"]["name"])
        best_model.encoder = best_model.encoder.from_pretrained("poly-encoder-neg-60")
        best_model.tokenizer = best_model.tokenizer.from_pretrained("poly-encoder-neg-60")
